# RadioFable architecture

RadioFable is a small, client-only React application. `useRadioFable` owns the active load, filters, and selection set. Components receive state and callbacks, while `lib/filters.ts` keeps filtering deterministic and easy to test.
